                        ...
                        <div class="box-body">
							...
							<div class="row pt-3">
								...
								<template v-if="!isChecque">
									...
									<div class="col-md-2">
										<label class="form-label">Change </label><br>
										<div class="input-group">
                                    ...
                                    <div class="row"  style="overflow-y: scroll; min-height: 100px; height: 500px;" >
                                        <div class="col-md-3 col-xs-6" v-for="product in requiredData.allProducts" :key="product.id"  disabled="disabled" data-toggle="tooltip" title="" style="padding-left: 5px; padding-right: 5px;" data-original-title="Rado Watch">
                                            <div
                                                ...
                                                style="max-height: 150px; min-height: 160px; cursor: pointer; background-color: #a1db75;"
                                                >
                                                ...
                                                <div class="box-body box-profile">
                                                    ...
                                                    <label class="text-center" style="font-weight: bold; font-family: sans-serif;">
        ...
        methods: {
			...
			submitSale(formObject){
				...
				let URL = baseUrl +'/'+ "sale";
                    this.axios.post(URL,_this.formObject)
                    .then((response) => {
						_this.$toastr('success','New sales inserted successfully', 'Success');
						let sales = response.data.result[0];
						let salesDetails = response.data.result[1];
 					 	var product = "";
						salesDetails.forEach(element => {
							product += '<table>';
							product += '<td> '+element.product.item_code+'</td>';
							product += '<td> '+element.quantity+'</td>';
							product += '<td> '+element.sell_amount+'</td>';
							product += '<td> '+element.total+'</td>';
							product += '</table>';
							return product;
						});
						...
						<body>
							${`<div id='printContainer'>
								<h2 style="margin-top:0px; margin-bottom:0px;" class="text-center">Inventory Management System</h2>
								...
								<table>
									...
									<tr>
										<td>Created At : </td>
										<td><b>${sales.created_at}<br></b></td>
									</tr>
									<tr>
										<td>Customer Name :</td>
										<td><b>${sales.customer.name}</b></td>
									</tr>
									<tr>
										<td>Mobile : </td>
										<td><b>${sales.customer.mobile} </b></td>
									</tr>
			...
			payableAmount(){
				...
				let changeAmount = grandTotal-parseInt(payableAmount);
			...
			productSearch(){
				const _this = this;
				let itemCode = _this.formObject.item_code;
				 let URL = baseUrl +'/'+ "get-by-all-product-by-stock?product_id=" + product_id;
                    this.axios.get(URL)
                    .then((response) => {
