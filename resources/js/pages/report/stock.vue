        ...
        <div class="panel panel-default">
            <div class="d-flex align-items-center justify-content-between">
                ...
                <div class="dt-buttons">          
                    ...
                    <button class="btn btn-info" type="button" @click="printStock()">
            ...
            <div class="table-responsive">
                <table id="dataTable" class="table table-striped table-bordered dataTable" cellspacing="0" width="100%" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
                    ...
                    <tbody>
                        <tr role="row" class="odd" v-for="(productData, productIndex) in requiredData.allProductsSearch" :key="productIndex" >
                            <td class="sorting_1">{{ productData.name }}</td>
                            <td>{{ productData.item_code }}</td>
                            <td>{{ productData.sell_price }}</td>
                            <td>{{ productData.stockIn-productData.stockOut }}</td>
                        </tr>
...
export default {
    ...
    methods: {
        downloadFile(){
            this.$http.get( baseUrl +'/'+ "create-pdf", {responseType: 'arraybuffer'})
                .then(response => {
         ...
         printStock(){
             const _this = this;
            let details = _this.requiredData.allStock;
            var product = "";
            var total = 0;
            var price = 0;
            details.forEach(element => {
                        ...
                        product += '<td class="item"> '+(parseInt(element.stockIn)-parseInt(element.stockOut))+'</td>';
                        product += '</tr>';
                        price += element.sell_price;
                        total  += (parseInt(element.stockIn)-parseInt(element.stockOut));
                        return product;
				});
    ...
    mounted() {
         this.getDataList();
         this.getGeneralData(["allProductsSearch","allStock"]);
    },
}
</script>
